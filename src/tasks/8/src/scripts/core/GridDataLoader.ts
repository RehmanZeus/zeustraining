...
export class GridDataLoader {
    ...
    loadJSONData<T>(dataArray: T[]): void {
        ...
        for (let col = 0; col < columnNames.length; col++) {
            const cell = this.gridMatrix.getCell(1, col + 1)
            if(!cell) continue;
            cell.data = columnNames[col].toUpperCase();
        }
        ...
        for (let row = 0; row < dataArray.length; row++) {
            ...
            for (let col = 0; col < columnNames.length; col++) {
                ...
                const cell = this.gridMatrix.getCell(row+2, col + 1)
                if(!cell) continue;
                cell.data = cellValue; // <-- row+2
            }
        }
    }
}